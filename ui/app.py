...
def init_state():
	...
	if "available_topics" not in st.session_state:
		...
		fresh = fetch_topics()
		for t in fresh:
			if t not in st.session_state.available_topics: st.session_state.available_topics.append(t)
	if "active_topic" not in st.session_state:
		st.session_state.active_topic = st.session_state.available_topics[0] if st.session_state.available_topics else "General"
	for key in ["trigger_send", "last_upload_status", "pending_interrupt"]:
		if key not in st.session_state: st.session_state[key] = None
	if "history_threads" not in st.session_state: st.session_state.history_threads = []
...
def render_sidebar():
	with st.sidebar:
		...
		for topic in st.session_state.available_topics:
			...
			if st.button(f"{'📂' if is_active else '📁'} {topic}", key=f"topic_{topic}", type="primary" if is_active else "secondary", use_container_width=True):
				if not is_active:
					st.session_state.active_topic = topic
					st.session_state.messages = []
					st.rerun()

		new_topic = st.text_input("new_topic", placeholder="+ New topic…", label_visibility="collapsed", key="new_topic_input")
		...
		if st.session_state.last_upload_status:
			st.success(st.session_state.last_upload_status[1]) if st.session_state.last_upload_status[0] == "success" else st.warning(st.session_state.last_upload_status[1])
		...
		if uploaded_file and st.button("🚀 Process & Index", use_container_width=True, type="primary"):
			with st.spinner("Indexing..."):
				try:
					res = requests.post(f"{API_URL}/upload", files={"file": (uploaded_file.name, uploaded_file.getvalue(), "application/pdf")}, data={"topic": st.session_state.active_topic}, timeout=300)
					if res.status_code == 200:
		...
		st.divider()
		if st.button("📜 View History", use_container_width=True): show_history_dialog()
...
def render_chat():
	...
	if not st.session_state.messages:
		st.markdown(f"<div class='welcome-card'><div class='welcome-orb'>⚡</div><div style='margin-bottom:1.25rem;'><span class='topic-badge'>📂 {active}</span></div><h2 class='welcome-title'>What would you like to know?</h2><p class='welcome-sub'>Ask anything — I'll search, rerank, and synthesize an accurate answer with citations.</p></div>", unsafe_allow_html=True)
		cols = st.columns(3)
		prompts = [("📊 Summarize findings", "Summarize the key findings."), ("🔍 Main topics?", "What are the main topics?"), ("📋 Key policies", "List the key policies.")]
		for col, (label, p) in zip(cols, prompts):
			with col:
...
def show_history_dialog():
	st.markdown("<p style='font-size:0.85rem;color:#64748b;margin-bottom:1.5rem;'>Select a previous conversation to resume.</p>", unsafe_allow_html=True)
	...
	if not threads: st.info("No history found."); return
		...
		for thread in threads:
			...
			with c2:
				if st.button("🗑️", key=f"del_{tid}"):
					if requests.delete(f"{API_URL}/history/{tid}", timeout=5).status_code == 200: st.rerun()
