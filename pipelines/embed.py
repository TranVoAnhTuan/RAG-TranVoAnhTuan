...
async def embed_and_load_node(state: IngestionState) -> dict:
    chunks = state["chunks"]
    ...
    try:
        ...
        if texts:
            ...
            for i, item in enumerate([c for c in chunks if c["content"]]):
                ...
                if sparse_dict and len(sparse_dict.indices) > 0:
                    indices = (
                        sparse_dict.indices.tolist()
                        if hasattr(sparse_dict.indices, "tolist")
                        else list(sparse_dict.indices)
                    )
                    values = (
                        sparse_dict.values.tolist()
                        if hasattr(sparse_dict.values, "tolist")
                        else list(sparse_dict.values)
                    )
            ...
            if points:
                ...
                for i in range(0, len(points), batch_size):
                    ...
                    tasks.append(client.upsert(collection_name=settings.QDRANT_COLLECTION_NAME, points=batch))

                logger.info(
                    f"--- [CHECKPOINT 6] Uploading to Qdrant in {len(tasks)} batches (Batch size: {batch_size} points)... ---"
                )
        ...
        return {"status": f"Failed with error: {e!s}"}
